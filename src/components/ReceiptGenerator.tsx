...
export function ReceiptGenerator({
  ...
  const updateQuantity = (serviceId: string, quantity: number) => {
    const safeQuantity = Math.min(50, Math.max(0, Math.floor(quantity)));
      ...
      setSelectedItems(
        selectedItems.map((item) =>
          item.serviceId === serviceId
            ? { ...item, quantity: safeQuantity, total: safeQuantity * item.price }
            : item
        )
      );
    }
  ...
  const sanitizePhone = (value: string) => {
    ...
    return `${hasLeadingPlus ? '+' : ''}${digitsOnly}`.slice(0, 16);
  };
  ...
  const isValidPhone = (value: string) =>
    value.length === 0 || /^\+?\d{7,15}$/.test(value);

  const getRateLimitMessage = () => {
    const now = Date.now();
    const lastSubmissionMs = Number(localStorage.getItem('last_quote_submission_ms') || '0');
    const minimumGapMs = 30_000;
    const remainingMs = minimumGapMs - (now - lastSubmissionMs);
    if (remainingMs > 0) {
      return `Please wait ${Math.ceil(remainingMs / 1000)} seconds before sending another quote.`;
    }

    const dayKey = new Date().toISOString().split('T')[0];
    const submissionDay = localStorage.getItem('quote_submission_day');
    const dailyCount = Number(localStorage.getItem('quote_submission_count') || '0');
    const maxDailySubmissions = 30;

    if (submissionDay === dayKey && dailyCount >= maxDailySubmissions) {
      return 'Daily quote limit reached. Please try again tomorrow.';
    }

    return null;
  };

  const persistSubmissionCounters = () => {
    const now = Date.now();
    const dayKey = new Date().toISOString().split('T')[0];
    const submissionDay = localStorage.getItem('quote_submission_day');
    const currentCount = Number(localStorage.getItem('quote_submission_count') || '0');

    localStorage.setItem('last_quote_submission_ms', String(now));
    if (submissionDay === dayKey) {
      localStorage.setItem('quote_submission_count', String(currentCount + 1));
    } else {
      localStorage.setItem('quote_submission_day', dayKey);
      localStorage.setItem('quote_submission_count', '1');
    }
  };
  ...
  const handleSendWhatsApp = async () => {
    ...
    const cleanCustomerName = sanitizeText(customerName || 'Walk-in Customer', 80);
    ...
    const cleanEventType = sanitizeText(eventType, 40) || 'Event';
    ...
    if (cleanEventDate && cleanEventDate < today) {
    ...
    const taxRate = Number.isFinite(settings.taxRate) ? Math.max(0, settings.taxRate) : 0;
    ...
    try {
      ...
      await onSendWhatsApp(receipt);
      persistSubmissionCounters();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-8">
        ...
        >
          ...
          >
            ...
            {isSubmitting ? 'Sending...' : 'Send Receipt on WhatsApp'}
          </motion.button>
        </motion.div>
      </div>
    </div>
